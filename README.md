...
|[:white_check_mark:](https://github.com/davizuku/oregex/blob/master/src/Matchers/PositiveLookBehindMatcher.hpp)|PositiveLookBehind|Matches the elements in list without consuming the input.| `abc` -> `/(?<=bc)abc/` :white_check_mark:<br>`abc` -> `/(?<=a)abc/` :x:|
